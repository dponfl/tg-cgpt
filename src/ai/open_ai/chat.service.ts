...
export class OpenAiChatService implements IAIText {
	...
	public textRequest(userGuid: string, messages: IOpenAiChatMessage[]): Promise<AiTextResponse> {
		...
		return new Promise(async (resolve, reject) => {
			...
			try {
				...
				let finishTimeOriginal;

				const timeOutId = setTimeout(() => {

					finishTimeOriginal = new Date().getTime();
					...
					const requestMetricsRecSuccess: IRequestsTable = {
						...
						duration: finishTimeOriginal - startTimeOriginal,
					};
				...
				const startTimeOriginal = new Date().getTime();
